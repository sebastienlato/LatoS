...
DIR = Path(os.environ.get("LATOS_SUBSET_TEST_TOOLS", ROOT / "experiments/phase-17-1/small-subset"))
sys.path.insert(0, str(DIR))
import ss_adapter as a  # noqa: E402
import ss_common as c  # noqa: E402
import ss_controller as controller  # noqa: E402
import ss_evidence as evidence  # noqa: E402
import ss_worker as worker  # noqa: E402
...
def test_fixed_accounting():
    ...
    assert list(c.ARMS) == ["A", "B", "C"]
    assert set(c.ENDPOINTS) == {"A1", "B1", "B2", "C2"}
...
def test_seed_order_and_pass_switch():
    ...
    one, two = ShuffleStream(6105, 160), ShuffleStream(6105, 160)
...
def test_schedule_contrast_and_dimensions():
    ...
    short = TrainingConfig(max_steps=382, warmup_steps=19, learning_rate=0.0003, min_lr_ratio=0.1)
    long = dataclasses.replace(short, max_steps=764)
    assert short.learning_rate_at(19) == long.learning_rate_at(19)
    assert short.learning_rate_at(382) < long.learning_rate_at(382)
    assert short.learning_rate_at(382) == long.learning_rate_at(764)
...
def test_deadline_includes_preparation():
    ...
    assert sum(c.STAGES.values()) == 2400 and c.TOTAL == 2700
...
def test_no_final_or_old_training_entrypoint():
    ...
    assert all(s[1] <= 2 for s in c.ARMS.values())
...
def test_diagnostic_checkpoint_roundtrip_no_optimizer(tmp_path):
    ...
    assert saved["readback_exact"] and saved["kind"] == "small-subset-no-resume-v1"
...
def test_complete_scripted_evidence_and_tamper_rejection(tmp_path, monkeypatch):
    """1,910 fabricated records exercise the verifier, never an optimizer/model."""
    import hashlib
    import shutil
    ...
    for arm, (_, passes, steps) in c.ARMS.items():
        ...
        config = TrainingConfig(
            max_steps=steps, warmup_steps=19, learning_rate=0.0003, min_lr_ratio=0.1
        )
        rates = [config.learning_rate_at(i) for i in range(1, steps + 1)]
        c.write(job / "schedule.json", rates)
        initial = {"scripted": True}
        ...
        gen = torch.Generator().manual_seed(160)
        total = 0
        records = []
    ...
    assert evidence.verify_training(tmp_path)["serious_updates"] == 1910
    trace = tmp_path / "jobs/A/updates.jsonl"
...
def test_controller_fixed_sequence_and_failure_stop(tmp_path, monkeypatch):
    ...
    manifest = {"files": {"ss_controller.py": {"sha256": "fixed"}}}
...
def test_failed_study_packages_existing_evidence_only(tmp_path, monkeypatch):
    ...
    with zipfile.ZipFile(root / "small-subset-return.zip") as z:
...
def test_return_rejects_over_budget_before_reading_payloads(tmp_path):
    spec = importlib.util.spec_from_file_location(
        "subset_review", ROOT / "experiments/phase-17-1/small-subset/verify_return.py"
    )
    ...
    c.write(
        completed,
        {
            ...
            "total_elapsed_seconds": 2690,
            "execution_complete": True,
        },
    )
    guardian = tmp_path / "guardian.json"
    c.write(guardian, {"total_wall_seconds": 2701, "exit_code": 0})
