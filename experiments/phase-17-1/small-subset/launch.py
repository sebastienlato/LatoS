...
def bootstrap(args):
    ...
    root = args.transfer.resolve() / "small-subset-v1"
    with (root / "bootstrap.log").open("x", encoding="utf-8") as log:
        ...
        try:
            ...
            destination.mkdir()
            with zipfile.ZipFile(args.archive) as z:
                for name in ("ss_common.py", "ss_controller.py", "ss_evidence.py"):
                    (destination / name).write_bytes(z.read(name))
            sys.path.insert(0, str(destination))
            from ss_controller import run
...
def main():
    ...
    try:
        result = guarded(command, began + 2695)
