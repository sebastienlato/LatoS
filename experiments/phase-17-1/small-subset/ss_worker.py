...
def evaluate(request, bundle, transfer, output, guard):
    ...
    if name == "reference":
        ...
        require(
            saved["kind"] == "small-subset-no-resume-v1" and saved["readback_exact"],
...
def execute(req, bundle, transfer, output, job):
    ...
    if fixture:
        ...
        from ss_adapter import advance_pass, check_progress, save_checkpoint, update
        ...
        require(cfg.parameter_count == (21238272 if arm == "C" else 34087424), "Model count")
        ...
        data = datasets(bundle)
        legacy.seed_all(160)
        trainer = Trainer(create_model(cfg, 160), training, *data, "cuda", precision="bfloat16")
    ...
    if not fixture:
        if req["arm"] == "B":
            a = read(output / "jobs/A/initial.json")["state"]
            require(
                initial["model"] == a["model"] and initial["sampler"] == a["sampler"],
                "A/B initialization or data ordering differs",
            )
