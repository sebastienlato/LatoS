...
def extract(archive, tools, digest):
    ...
    require(manifest["attempt"] == "small-subset-v1", "Wrong study")
    return manifest
...
def run(args):
    ...
    deadline = started + 2690  # Outer OS job kills the full process tree before 45 minutes.
    ...
    try:
        ...
        require(
            sha(Path(__file__)) == manifest["files"]["ss_controller.py"]["sha256"],
        ...
        require(
            ...
            and auth["attempt"] == "small-subset-v1"
            and auth["proposal_sha256"] == sha(tools / "PROPOSAL.md")
            and auth["hard_wall_seconds"] == 2700
        ...
        def dispatch(spec, stop):
            ...
            child(
                [str(python), str(tools / "ss_worker.py"), str(path)],
                env,
                root / (spec["id"] + ".log"),
                stop,
                root.parent,
        ...
        stop = min(deadline - 480 - 300, time.monotonic() + STAGES["train"])
        ...
        stop = min(deadline - 300, time.monotonic() + STAGES["evaluate"])
    ...
    script = (
        tools / "ss_evidence.py"
        if (tools / "ss_evidence.py").exists()
        else Path(__file__).with_name("ss_evidence.py")
    )
