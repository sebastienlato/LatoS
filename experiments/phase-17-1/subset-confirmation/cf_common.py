...
SEED = 161
PARAMETERS = 21238272
CONTRASTS = (("D2", "E2"), ("E2", "E4"), ("D2", "E4"))
ARMS = {"D": (4, 2, 764), "E": (4, 4, 1528)}
ENDPOINTS = {"D2": ("D", 764), "E2": ("E", 764), "E4": ("E", 1528)}
STAGES = {"prepare": 180, "train": 1080, "evaluate": 300, "package": 180}
TOTAL = 1800
...
def selfcheck():
    ...
    require(sum(a[2] for a in ARMS.values()) == 2292, "Updates")
    require(sum(a[1] for a in ARMS.values()) * TRAIN_TARGETS == 11658090, "Exposure")
    require(sum(STAGES.values()) + 60 == TOTAL, "Cumulative budget")
    require(boundary(1528) == (3, 6105, 24420), "Fourth boundary")
    require(SEED == 161 and PARAMETERS == 21238272, "Frozen seed/model")
    return {"passed": True, "optimizer_updates": 0, "kind": "scripted-accounting"}


def terminal_disposition(passed):
    require(type(passed) is bool, "Terminal gate status must be Boolean")
    return {
        "terminal_endpoint": "E4",
        "fixed_subset_direction": (
            "stop/owner-review-development-pass-only" if passed else "stop/rethink-data-training"
        ),
